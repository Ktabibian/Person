...
public class PersonHandler {
    ...
    public String whileLoop() {
        ...
        // while `counter` is less than length of array
            while(counter < personArray.length) { // begin loop
                ...
                currentPerson = personArray[counter]; // use `counter` to identify the `current Person` in the array
                result = result + currentPerson.toString(); // get `string Representation` of `currentPerson`
                counter++; // append `stringRepresentation` to `result` variable
    ...
    public String forLoop() {
        ...
        for(int counter = 0; counter < personArray.length; counter++){
            Person personNow = personArray[counter];
            result += personNow.toString();
        }
